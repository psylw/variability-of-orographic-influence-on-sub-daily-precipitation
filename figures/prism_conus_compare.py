...
p = xr.open_dataset('../output/prism_season_accum_temp.nc')
c = xr.open_dataset('../output/conus404_season_accum_temp.nc')
...
levels_diff = [-150,-100, -50, -25, 0, 25, 50, 100,150]
...
for i, season in enumerate(seasons):
    ...
    # Compute difference
    diff = ds_son.PREC_ACC_NC.mean(dim='year_season') - ds_son_p.band_data.mean(dim='year_season')
    ...
    gdf2.plot(ax=ax_diff, edgecolor='red', facecolor='none',linestyle='--')
    ...
    im_son = abs((diff/ds_son.PREC_ACC_NC.mean(dim='year_season'))).plot(
        ax=ax_son,
        cmap=cmap_son,
        norm=norm_son,
    ...
    ax_son.text(-104.4, 39.6, "10", fontsize=14, color='black', ha='center')
    ax_son.text(-104.7, 37.1, "11", fontsize=14, color='black', ha='center')
    ax_son.text(-106,   37.4, "13", fontsize=14, color='black', ha='center')
    ax_son.text(-108.6, 40.5, "14", fontsize=14, color='black', ha='center')
...
fig.savefig("../figures_output/prism_compare.pdf",bbox_inches='tight',dpi=600,transparent=False,facecolor='white')
# %%
